...
static uint32_t g_avg_peak_volume_measurements_count = 0;
static uint16_t g_last_peak_volume = 0;
static time_t g_last_avg_peak_volume_rst_time = 0;
...
uint16_t get_current_peak_volume() {
  ...
  g_avg_peak_volume = (g_avg_peak_volume + current_peak_volume) / ++g_avg_peak_volume_measurements_count;

  if (millis() - g_last_avg_peak_volume_rst_time >= AVG_VOLUME_RESET_INTERVAL_MS) {
    g_last_avg_peak_volume_rst_time = (time_t)millis();
    g_avg_peak_volume = 0;
    g_avg_peak_volume_measurements_count = 0;
  }
